...
func ConvertItems(items []parser.Item, feed orm.Feeds) []orm.Item {
	result := make([]orm.Item, len(items))
...
func listItems(dao *daos.Dao, f orm.Feeds) ([]orm.Item, error) {
	lmod := ""
	etag := ""
	if state := orm.GetHTTPState(dao, f.Id); state != nil {
	...
	if lmod != "" || etag != "" {
		orm.SetHTTPState(dao, f.Id, lmod, etag)
	}
